...
function Addbooks() {
  ...
  const formik = useFormik({
    ...
    validate: (values) => {
      ...
      if (
        values.user_name &&
        (values.user_name.length <= 2 || values.user_name.length > 15)
      ) {
      ...
      if (
        ...
        (values.mobile_no && /[A-Z._%+-]/i.test(values.mobile_no))
      ) {
    ...
    onSubmit: async (values) => {
      try {
        ...
        if (
          ...
          server.data.message ===
            "User mobile number already there, use another" ||
          server.data.message ===
            "User email & mobile number already there, use another"
        ) {
      ...
      <div className={styles._bg}>
        ...
        <form className={styles.form}>
          <div className={styles.threeinput_col}>
            ...
            <p>*** Before creating get any id proof ***</p>
          </div>
        ...
        <Link to="/user" className={"forback ${style.forgot}"}>
          Back
        </Link>
      </div>
